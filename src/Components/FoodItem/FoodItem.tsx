    ...
    >
      ...
      >
        <div className={s.wrapperContent}>
          <div
            className={
              state.inStock ? s.topTitle : `${s.topTitle} ${s.textBlocked}`
            }
          >
            {isMouseOff ? (
          ...
          <div
            className={
              state.inStock
                ? s.productName
                : `${s.productName} ${s.textBlocked}`
            }
          >
            {state.productName}
          </div>
          <div
            className={state.inStock ? s.taste : `${s.taste} ${s.textBlocked}`}
          >
            {state.taste}
          </div>
          <div
            className={
              state.inStock
                ? `${s.portions} ${s.topPortions}`
                : `${s.portions} ${s.topPortions} ${s.textBlocked}`
            }
          >
            <span>{state.portions}</span> порций
          </div>
          <div
            className={
              state.inStock
                ? `${s.portions} ${s.botPortions}`
                : `${s.portions} ${s.botPortions} ${s.textBlocked}`
            }
          >
            <span>{state.presentsMouse === 1 ? "" : state.presentsMouse}</span>
            {state.presentsMouse <= 4 ? " мыши" : " мышей"} в подарок
          </div>
          <div
            className={
              state.inStock
          ...
          <img
            className={state.inStock ? s.photo : `${s.photo} ${s.photoBlocked}`}
            src={Photo}
          />
          <div
            className={
              state.inStock
                ? s.weightWrapper
                : `${s.weightWrapper} ${s.weightBlocked}`
            }
          >
